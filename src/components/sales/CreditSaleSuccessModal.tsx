...
export default function CreditSaleSuccessModal({
    result,
    customerName,
    onClose,
}: CreditSaleSuccessModalProps) {
    ...
    /** WhatsApp deep-link per SRS §6.11.1 — no Business API needed */
    const whatsappLink = `https://wa.me/?text=${encodeURIComponent(
        `আসসালামু আলাইকুম ${customerName}, আপনার বাকী ${formatTk(result.totalAmount)} টাকা (${result.invoiceNumber})। অনুগ্রহ করে শীঘ্রই পরিশোধ করুন। - DokaniAI`,
    )}`;
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* ── Customer Info ── */}
                <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm">
                    <div className="flex items-center gap-2">
                        <span className="material-symbols-outlined text-lg text-amber-700">
                            person
                        </span>
                        <span className="font-semibold text-amber-900">
                            {customerName || "অজানা ক্রেতা"}
                        </span>
                    </div>
                ...
                <div className="mt-4 space-y-2">
                    {/* WhatsApp Reminder — SRS §6.11.1 deep-link */}
                    <a
                        href={whatsappLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] py-2.5 text-sm font-semibold text-white transition hover:bg-[#1da851] active:scale-[0.98]"
                    >
                        ...
                        WhatsApp রিমাইন্ডার পাঠান
                    </a>

                    {/* Close button */}
                    <button
                        onClick={handleClose}
                        className="w-full rounded-xl bg-amber-600 py-2.5 text-sm font-semibold text-white transition hover:bg-amber-700 active:scale-[0.98]"
                    >
                        ঠিক আছে
                    </button>
                </div>
