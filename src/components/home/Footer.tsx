    ...
    <footer className="bg-surface-container-low w-full mt-20">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 px-6 py-12 max-w-7xl mx-auto">
        <div className="col-span-2 md:col-span-1 space-y-4">
          ...
          <p className="text-sm text-on-surface-variant font-medium">{t("copyright")}</p>
        </div>
        
        <div className="space-y-3">
          <h5 className="font-bold text-primary">{t("plans")}</h5>
          <ul className="text-sm text-on-surface-variant font-medium space-y-2">
            <li><Link className="hover:text-primary hover:translate-x-1 transition-transform inline-block" href="#">{t("trialPlan")}</Link></li>
            <li><Link className="hover:text-primary hover:translate-x-1 transition-transform inline-block" href="#">{t("basicPlan")}</Link></li>
            <li><Link className="hover:text-primary hover:translate-x-1 transition-transform inline-block" href="#">{t("proPlan")}</Link></li>
          </ul>
        </div>
        
        <div className="space-y-3">
          <h5 className="font-bold text-primary">{t("resources")}</h5>
          <ul className="text-sm text-on-surface-variant font-medium space-y-2">
            <li><Link className="hover:text-primary hover:translate-x-1 transition-transform inline-block" href="#">{t("privacyPolicy")}</Link></li>
            <li><Link className="hover:text-primary hover:translate-x-1 transition-transform inline-block" href="#">{t("pwaGuide")}</Link></li>
            <li><Link className="hover:text-primary hover:translate-x-1 transition-transform inline-block" href="#">{t("helpCenter")}</Link></li>
          </ul>
        </div>
